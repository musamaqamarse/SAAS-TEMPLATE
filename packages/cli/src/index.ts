...
import { scaffold, type ScaffoldConfig, type ScaffoldEvent } from "@create-saas/core";
...
function readPackageVersion(): string {
  ...
  // (which advertises the bin). Works in dev (src/) and after build (dist/).
  let dir = __dirname;
  for (let i = 0; i < 4; i++) {
    ...
    if (fs.existsSync(candidate)) {
      try {
        ...
        if (pkg.name === "create-saas") return pkg.version ?? "unknown";
      } catch {
